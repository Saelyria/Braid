...
extension Diff.Element {
    init(_ diffElement: SimpleDiff.Element) {
        switch diffElement {
        case let .delete(at):
            self = .delete(at: at)
        case let .insert(at):
...
extension Collection {
    ...
    func extendedDiff(_ other: Self, isEqual: EqualityChecker<Self>) -> Diff {
    ...
    ///   - isEqual: instance comparator closure
    /// - Returns: ExtendedDiff between the callee and `other` collection
    func extendedDiff(from diff: SimpleDiff, other: Self, isEqual: EqualityChecker<Self>) -> Diff {
        ...
        for candidateIndex in diff.indices {
            if !moveTargetIndices.contains(candidateIndex) && !moveOriginIndices.contains(candidateIndex) {
                ...
                if let match = match {
                    ...
                    sourceIndex.append(candidateIndex)
                    elements.append(Diff.Element(candidate))
                }
            }
        }
    ...
    func firstMatch(
        _ diff: SimpleDiff,
        dirtyIndices: Set<SimpleDiff.Index>,
        candidate: SimpleDiff.Element,
        candidateIndex: SimpleDiff.Index,
        other: Self,
        isEqual: EqualityChecker<Self>
        ) -> (Diff.Element, SimpleDiff.Index)? {
    ...
    func createMatch(_ candidate: SimpleDiff.Element, match: SimpleDiff.Element, other: Self, isEqual: EqualityChecker<Self>) -> Diff.Element? {
...
extension Collection where Element: Equatable {
    ...
    public func extendedDiff(_ other: Self) -> Diff {
