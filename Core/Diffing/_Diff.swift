...
//    func diffableItems(for section: S) -> [CollectionIdentifiable]? {
//        if let viewModels = self.sectionCellViewModels[section] as? [CollectionIdentifiable] {
//            return viewModels
//        } else if let models  = self.sectionCellModels[section] as? [CollectionIdentifiable] {
//            return models
//        } else if self.sectionCellViewModels.isEmpty && self.sectionCellModels.isEmpty {
//            return [] // make sure to assume the data model is diffable if it's empty
//        }
//        return nil
//    }
//}
...
//        -> [SectionChange]
//    {
//        func index(for section: S) -> Int? {
//            return new.displayedSections.firstIndex(of: section)
//        }
//
//        var diff: [SectionChange] = []
//
//        // add the diff of each section to the full nested diff
...
//        })
//
//        // get all sections that were involved by changes and iterate over them
//        var involvedSections: Set<S> = Set(old.displayedSections)
//        involvedSections.formUnion(new.displayedSections)
//
//        for section in involvedSections {
//            if let oldItems = old.diffableItems(for: section), let newItems = new.diffableItems(for: section) {
//                let sectionChanges = self.diff(old: oldItems, new: newItems)
//                diff.append(contentsOf: sectionChanges.map { change -> SectionChange in
//                    switch change {
//                    case .delete(let index):
//
//                        return .deleteSection(index: index)
...
//                })
//            } else {
//                // the section wasn't diffable; just reload the entire section
//
//            }
//        }
//
