 ...
 /// ```
 /// - SeeAlso: Diff
 struct SimpleDiff: DiffProtocol {
 ...
 extension Collection {
    ...
    func diff(
        ...
        return SimpleDiff(elements:
            diffPath
                .compactMap { SimpleDiff.Element(trace: $0) }
        )
    }
 ...
 extension SimpleDiff {
    init(traces: [Trace]) {
        elements = traces.compactMap { SimpleDiff.Element(trace: $0) }
    }
 }
 
 extension SimpleDiff.Element: CustomDebugStringConvertible {
 ...
 extension SimpleDiff: ExpressibleByArrayLiteral {
    
    init(arrayLiteral elements: SimpleDiff.Element...) {
        self.elements = elements
    }
 }
