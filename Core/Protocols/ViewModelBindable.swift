...
/**
 ...
 view model, bind that data to itself. The object conforming to this protocol declares what its view model type is with
 its associated `ViewModel` type.
 */
public protocol ViewModelBindable {
    /// The type that this object's view model must be.
    associatedtype ViewModel: CollectionIdentifiable
